...
def get_soil_vars(xr_df:Union[DataArray, Dataset]=None, 
    ...
    xy_list = [lonlat2xy(row['lon'], row['lat'], x_corner, y_corner, cellsize) \
               for _, row in tqdm(elev_df.iterrows(), total=len(elev_df))]

    elev_df['arc_xy'] = xy_list


    def add_value(elev_df, path, name, dtype='int'):
...
def get_water_cover(xr_df:Union[DataArray, Dataset]=None, 
                    df:Union[pd.DataFrame, None]=None, 
                    countries:list = None, 
                    invert =True):
    """
    ...
    if ((df is None) & (xr_df is None)):
        print('Using the default MERRA-2 grid for querying FAO HWSD')
        ds = xr.open_dataset(os.path.join(config['MERRA2']['path'], 
                                          'nasapower_vars.nc'), chunks=chunks)
        elev_df = ds[['lat','lon']].to_dataframe().reset_index()
    ...
    for land in land_covers:
        elev_df = add_value(elev_df, 
                            os.path.join(path, '{}_2000.asc'.format(land)),\
                            '{}_LAND'.format(land), dtype='float')
        elev_df[f'{land}_LAND'] = elev_df[f'{land}_LAND']
    ...
    return ds_el.transpose("lat","lon")["WAT_LAND"]
