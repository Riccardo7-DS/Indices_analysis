...
class WGCNModel(nn.Module):
    def __init__(self, args, model_config, data):
        super(WGCNModel, self).__init__()
        
        self.num_channels = model_config.num_samples+1 if model_config.include_lag is True else model_config.num_samples
        self.length = data.shape[-1]
        self.lag = args.feature_days

        self.data_bn = nn.BatchNorm1d(self.num_channels * self.length ) # channels (C) * vertices (V) ###

        self.l1 = TCN_GCN_unit(10, 16, self.length)
        self.l2 = TCN_GCN_unit(16, 32, self.length)
        self.l3 = TCN_GCN_unit(32, 64, self.length)

        self.conv_reduce_dim = unit_tcn(64, self.num_channels, kernel_size=1, stride=1) ###

        self.fc = nn.Linear( self.lag * self.num_channels, 1) # 4 * 30 * 5 = 600
    ...
    def forward(self, x):
        ...
        # x = x.view(x.size(0), -1)
        x = x.view(N, C*T, V).transpose(1, 2)
        x = self.fc(x)

        return x.view(N, 1, 1, V)